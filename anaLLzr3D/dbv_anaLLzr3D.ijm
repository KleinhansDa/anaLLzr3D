...
//	############################################### INITIALISE ###############################################
	saveSettings;
 //	Check plugins
	plugdir = getDirectory("plugins");
	plugdirlist = getFileList(plugdir);
	dep_mlj = false;
	dep_mcbi = false;
	dep_bf = false;
	dep_bar = false;
	dep_imgsc = false;
	for (i = 1; i < plugdirlist.length; i++) {
		if(startsWith(plugdirlist[i], "MorphoLibJ")) {
			dep_mlj = true;
		} 
		if(startsWith(plugdirlist[i], "mcib3d")) {
			dep_mcbi = true;
		}
		if(startsWith(plugdirlist[i], "BAR_")) {
			dep_bar = true;
		}
		if(startsWith(plugdirlist[i], "bio-formats")) {
			dep_bf = true;
		}
		if(startsWith(plugdirlist[i], "FeatureJ")) {
			dep_imgsc = true;
		}
	}
	if (dep_mlj&dep_mcbi&dep_bf&dep_bar&dep_imgsc == true) {
	} else {
		if  (dep_mlj == false) {dep_mlj = "not installed";} else {dep_mlj = "installed";}
		if  (dep_mcbi == false) {dep_mcbi = "not installed";} else {dep_mcbi = "installed";}
		if  (dep_bf == false) {dep_bf = "not installed";} else {dep_bf = "installed";}
		if  (dep_bar == false) {dep_bar = "not installed";} else {dep_bar = "installed";}
		if  (dep_imgsc == false) {dep_imgsc = "not installed";} else {dep_imgsc = "installed";}
		print("BAR:                    	"+dep_bar);
		print("Bio-Formats:        		"+dep_bf);
		print("MorphoLibJ (IJPB):      	"+dep_mlj);
		print("3D ImageJ Suite:  		"+dep_mcbi);
		print("ImageScience:  			"+dep_imgsc);
		print(" ");
		print("Go to 'Help' -> 'Update...' -> 'Manage update sites' \nActivate missing, then -> 'Close' -> 'Apply Changes'\n\nre-start FIJI and try again :)");
		exit("One or more update sites not active\nPlease Check Log Window");
	}
	//print(dep_mlj, dep_mcbi, dep_cb, dep_bf);
//	############################################### DIALOGS ###############################################
	cleanup();
	saveSettings();
	getDateAndTime(year, month, dayOfWeek, dayOfMonth, hour, minute, second, msec);
	print("anaLLzr3D" + " - time: "+ hour + ":" + minute+"\n");
	scrH = screenHeight();
		DiaH = scrH/8;
		InfoH = scrH/8;
		LogH = scrH/8;
...
//	Opening Dialog
	...
	Zorder = newArray("Bottom to Top", "Top to Bottom");
		...
		//Dialog.addMessage("~~~~~~~~~~~~~~~ DATA DIMENSIONS ~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
	  	  	//Dialog.addCheckbox("Multi Channel", false);							// 01	multi
	  	  	//Dialog.setInsets(0,20,0);
			//Dialog.addChoice("C1:", labelsC1);									// 02	C1
			//Dialog.setInsets(0,20,0);
			//Dialog.addChoice("C2:", labelsC2, " ");								// 03	C2
			//Dialog.setInsets(0,20,0);
			//Dialog.addCheckbox("Swap Channel #", false);							// 04	inv
			//Dialog.addCheckbox("Time-Series", false);								// 05	ts
		Dialog.setInsets(0,10,-10);
		Dialog.addMessage("----------------------- PROCESSING ----------------------");
			//Dialog.addCheckbox("Include deconvolution", false); 					// 06	decon
			formatchoice = newArray(".czi", ".nd2", ".tif", "other", "egal");
			Dialog.setInsets(20,10,-10);
			Dialog.addChoice("Input file format", formatchoice, ".tif");			// 07	format
			Dialog.setInsets(15,10,0);
			Dialog.addChoice("Z order", Zorder);									// 08	rev
			Dialog.setInsets(5,20,0);
			Dialog.addCheckbox("Register primordium", true); 						// 09	reg
			Dialog.addCheckbox("Save intermediates", false); 						// 10	meta
		...
		Dialog.addMessage("---------------------- THRESHOLDS ----------------------");
			...
			Dialog.addSlider("Segmentation", 0, 50, 5); 							// 13	tol
			//Dialog.addMessage("Minimum Volume");							
			Dialog.addSlider("Min. volume", 0, 50, 20);								// 14	vmin
			//Dialog.addMessage("Max. Volume threshold");							
			Dialog.addSlider("Max. volume", 250, 1000, 750);						// 15	vmax
		Dialog.setInsets(0,10,0);
...
//	Cell Constriction Dialog
	...
	if (cellAC) {
		...
		Dialog.create(header);
		Dialog.addCheckbox("Absolute ACI delta", false); 							// 19	fixAC
			Dialog.addSlider("µm", 1, 10, 2);										// 20	cellum
		Dialog.addCheckbox("Relative ACI delta", true); 							// 21	symAC
...
//	##################################################################################################################
	...
	for (i = 0; i < list.length; i++) {
		...
		if (!endsWith(list[i], format)) {
			exit("No more files to process with chosen file format")
		}
		if (endsWith(list[i], format)) {
		file = dir+list[i];
	...
	//	Create subdirectories
			...
			// 	datdir subdirectories
				// cells directory
				datcelldir = datdir + "cells" + File.separator;
					File.makeDirectory(datcelldir);
				// prims directory
				datprimdir = datdir + "prims" + File.separator;
					File.makeDirectory(datprimdir);
			if (rosAC) {
				datrosdir = posdir + "rosettes" + File.separator;
			...
			// 	metadata
			metadir = posdir + File.separator + "rois" + File.separator;
...
//	########################## Primordium Registration ############################## 
	...
	// Enlarge
		...
		roiManager("save selected", metadir + name + "_reg.zip");
...
//	############### Rosette Detection ##############
	...
	//  crop Primordium
	   	selectWindow(ZPSTDD);
	   	if (reg) {
	   		roiManager("Open", metadir + name + "_reg.zip");
	   		roiManager("select", 0);
	   		//if (ts) {
	   			//} else {
	   		run("Crop");
	...
	//	get normalised threshold value for point selection
		...
		roiManager("Add");
		roiManager("save", metadir + name + "_poc.zip");
	...
	//	put number of AC into array + print to log
		...
		setResult("AC", 0, Rlx);
		updateResults();
		saveAs("Results", datprimdir + "ros-count_" + name + ".txt");
		run("Clear Results");
	...
	// Measure Intensities along horizontal line
		...
		saveAs("Results", datprimdir + "ml-intens_" + name + ".txt");
		run("Clear Results");
		if (isOpen("Results")) {
	...
	//  Save capture
		...
		run("Select None");
		run("8-bit");
		roiManager("Open", metadir + name + "_poc.zip");
		run("Point Tool...", "type=Dot color=Green size=[Extra Large] label counter=0");
		run("Overlay Options...", "stroke=none width=0 fill=none set");
		//saveAs("Tiff", posdir + name + "_RC_ZPMAX_poc.tif");
		close();
		roiManager("reset");
		close(ZPSTDDG);
		close(ZPSTDD);
...
// 	###################### MORPHOLOGICAL SEGMENTATIONs ###########################
		...
		//	wait till segmentation is done
			...
			while (isOpen("Morphological Segmentation")) {
				...
				if (newTime > 10000) {
   					oldTime = getTime(); 
   					newTime = 0;
   					//print(elapsedTime/1000, " sec. elapsed");
   					loginfo = getInfo("log");
		...
		// ######## ########################################################################### #########
			...
			saveAs("results", datcelldir + "feret_" + name + ".txt");
			results();
			wait(100);
			}
		...
		// ######## ############################# Define Z values ############################# #########
			if (rosAC) {
				cellum = rosum; // I copy pasted the code, so rosum would be cellum
				Zslice = rosum/sizeZ;
				round(Zslice);
			}
		// ######## ############################ Cell Constriction ############################ #########
		...
		// ######## ###################### Rosette Constriction ####################### #########
			...
			// ######## Crop Rosette ########
	   			...
	   			n = nSlices();
	   			run("Properties...", "channels=1 slices="+n+" frames=1 unit=microns pixel_width="+sizeX+" pixel_height="+sizeY+" voxel_depth="+sizeZ);
	   			saveAs("Tiff", rosdir + name + "_OMap" + "_R" + pos + ".tif");
	   			R = getTitle();
	   		// ######## crop single cells and save to new dir ########
	   			selectWindow(R);
				run("3D Manager Options", "volume surface compactness fit_ellipse 3d_moments feret centroid_(pix) centroid_(unit) distance_to_surface centre_of_mass_(unit) bounding_box radial_distance surface_contact closest exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80");
	   		...
	   		// ### MEASURE SINGLE ROSETTE CELLS ###
	  			...
	  			Ext.Manager3D_DeselectAll(); // to refresh RoiManager
	   			...
	   			Ext.Manager3D_SaveResult("M", datrosdir + name + "_R" + pos + "_cells" + ".csv");
	   		...
	   		// ### MEASURE WHOLE ROSETTE MERGED ###
				...
				Ext.Manager3D_Measure();
				Ext.Manager3D_SaveResult("M", datrosdir + name + "_R" + pos + ".csv");
       			Ext.Manager3D_CloseResult("M");
       			Ext.Manager3D_Reset();
				Ext.Manager3D_Close();
				close(RP);
	   		...
	   		// ######## Draw Feret lines ########
				...
				Ext.Manager3D_Close();
				selectWindow(Ferret);
				saveAs("Tiff", rosdir + name + "_OMap" + "_R" + pos + "_FerretLines" + ".tif");
				FerretS = getTitle();
				run("Enhance Contrast...", "saturated=0.3 equalize process_all");
			...
			// check if there are actually any points
				List.setMeasurements; 
 				...
 				if (mean > 0) {
					...
					saveAs("XY Coordinates", datrosdir + name + "_R" + pos + "_XY.txt");
					getSelectionCoordinates(xpoints, ypoints);
					FX = xpoints;
					FY = ypoints;
					...
					Array.show("Results (Dist)",Dist);
					wait(100);
					saveAs("results", datrosdir + name + "_Dist_R" + pos + ".txt");
	   				results();
			...
			//updateResults();
	   		saveAs("results", datdir + name + "_ACM.txt");
	   		results();
	   		} // for all Rlx
	   		} // ifRlx >0
	   //} // if rosAC
    ...
    // CREATE COMPOSITE
	   ...
	   //close();
		...
		run("Open", "stack");
		run("3D Fill Holes");
		run("3D Manager Options", "volume surface centroid_(unit) centre_of_mass_(unit) radial_distance exclude_objects_on_edges_xy sync distance_between_centers=10 distance_max_contact=1.80 drawing=Contour");
		run("3D Manager");
		wait(100);
		Ext.Manager3D_Segment(128, 255);
		OMapP = getTitle();
		...
		Ext.Manager3D_Measure();
		Ext.Manager3D_SaveResult("M", datprimdir + name + ".csv");
       	Ext.Manager3D_CloseResult("M");
       	Ext.Manager3D_Reset();
		Ext.Manager3D_Close();
		close(OMapP);
...
//	get date and time
	...
	if (dayOfMonth<10) {
		day="0"+d2s(dayOfMonth, 0);
	} else {
		day=d2s(dayOfMonth, 0);
	}
//	finish
	print("finished: "+ hour + ":" + minute);
	selectWindow("Log");  //select Log-window 
	saveAs("Text", pardir + "log_anaLLyrz3D_"+year+month+day+".txt"); 
	images();
    results();
	roiManager("reset");
	setBatchMode(false);
...
function info() {
	html = "<html>"
	+"<h1><font color=purple>anaLLzr3D info log</h1>"
    +"<h2><b>Processing Options</b></h2>"
    +"<font size=+0>
    +"<ul>"
    	+"<li><i>Save intermediates</i><br>Saves intermediary processing steps for debugging</li>"
    	+"<li><i>Z order</i><br>Specify the order in which the Z-stack was recorded</li>"
    	+"<li><i>Register Primordium</i><br>Rotates the primordium to the horizontal midline and crops background<br>- saves registration roi<br>- saves rois and locations of points of constriction<br>- saves intensities along the horizontal midline</li>"
    	+"<li><i>Manual Segmentation</i><br>Create your own Ground Truth model</li>"
    +"</ul>"
    +"<h2><b>Threshold values</b></h2>"
    +"<font size=+0>
    +"<ul>"
    	+"<li><i>Membrane segmentation</i><br>The threshold level for Morphological Segmentation</li>"
    	+"<li><i>Min.Volume</i><br>Remove objects below the set threshold</li>"
    	+"<li><i>Max.Volume</i><br>Remove objects above the set threshold</li>"
    +"</ul>"
    +"<h2><b><i>Measurements</b></h2>"
    +"<font size=+0>
    +"<ul>"
    	+"<li><i>Apical Constriction</i><br>Measure the Apical Constriction of all cells based on<br>the model you can find at <i>Kleinhans & Lecaudey, 2019.</i></li>"
    	+"<li><i>Includes</i><br>- ellipse major<br>- ellipse minor<br>- feret angle<br>- distance from apical</li>"
    +"</ul>"
    +"</font>";
    Dialog.addHelp(html);
}
